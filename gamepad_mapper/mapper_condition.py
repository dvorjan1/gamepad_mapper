...
class MapperCondition:
    ...
    def _set_check(self, params, event):
        return event.code == params[0] and event.ev_type == "Key" and event.state == 1

    def _unset_check(self, params, event):
        return event.code == params[0] and event.ev_type == "Key" and event.state == 0

    def _equal_check(self, params, event):
        return (
            event.code == params[0]
            and event.ev_type == "Absolute"
            and event.state == int(params[1])
        )
